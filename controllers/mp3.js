...
//     mp3Url = response.data.Download_url;
//   })
    ...
    .then(() => {
      if (!req.body.name) {
        ...
        musicApi.search(req.body.name, "song").then((songResult) => {
          ...
          // const firstMatch = songResult.content.find((song) => {
          //   req.body.name.match(song.name);
          //   console.log(req.body.name.match(song.name));
          //   return song;
          // });
          ...
          fs.readdir(path.join(__dirname, "../music/lrc"), (err, data) => {
            if (data.indexOf(`${songName} - ${artistName}.lrc`) === -1) {
              ...
              const lrcObj = lrcParser(
                path.join(
                  __dirname,
                  `../music/lrc/${songName} - ${artistName}.lrc`
                )
              );
              ...
              //   )
              // );
              // const mp3Url = new Promise((resolve, reject) => {
              //   resolve(Mp3(songResult.content[0].videoId));
